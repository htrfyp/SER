...
import layer4_CNN_bilstm
from layer4_CNN_bilstm import layer4_CNN_bilstm
input_duration=3
...
while rideo_num<6:
    ...
    layer4_CNN_bilstm(x_traincnn, y_train,x_testcnn, y_test,rideo_num,feature_kind,opt)
    ...
    json_file = open("./Models/"+str(feature_kind)+"_BILSTM/CNN_ASR_MODEL"+str(rideo_num)+".json", 'r')
    ...
    loaded_model.load_weights("./Models/"+str(feature_kind)+"_BILSTM/CNN_ASR_MODEL"+str(rideo_num)+".h5")
    ...
    def print_confusion_matrix(name,confusion_matrix, class_names, figsize = (9,6), fontsize=14):
        ...
        plt.savefig('./Result/'+str(feature_kind)+'_BILSTM/True cs Predicted for '+str(name)+'rideo_num'+str(rideo_num)+'.png')
        plt.close('all')
    ...
    f = open('./Result/'+str(feature_kind)+'_BILSTM/accuracy for rideo_db'+str(rideo_num)+'.txt','w')
