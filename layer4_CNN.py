...
def layer4_CNN(x_traincnn, y_train,x_testcnn, y_test,rideo_num,feature_kind,opt):
    ...
    mcp_save = ModelCheckpoint('./Models/'+str(feature_kind)+'/CNN_ASR_MODEL'+str(rideo_num)+'.h5', save_best_only=True, monitor='val_categorical_accuracy')
    ...
    plt.savefig('./Result/'+str(feature_kind)+'/Accuracy for train and valid sets for rideo_db'+str(rideo_num)+'.png')
    plt.show()
    plt.close('all')
    f = open('./Result/'+str(feature_kind)+'/Accuracy for train and valid sets for rideo_db'+str(rideo_num)+'.txt','w')
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('./Result/'+str(feature_kind)+'/Loss for train and valid sets for rideo_db'+str(rideo_num)+'.png')
    plt.show()
    plt.close('all')
    f = open('./Result/'+str(feature_kind)+'/Loss for train and valid sets for rideo_db'+str(rideo_num)+'.txt','w')
    ...
    with open("./Models/"+str(feature_kind)+"/CNN_ASR_MODEL"+str(rideo_num)+".json", "w") as json_file:
